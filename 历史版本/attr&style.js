...
$.fn.extend( {
    ...
    _attr: function( attr, val ){
        ...
        * 2、如果为1
        * 3、继续判断attr是不是字符串，如果是，获取第一个元素指定的属性节点值返回
        * 4、如果不是，继续判断是不是对象，如果是，遍历这个对象，得到所有的属性节点值
        ...
        if( arguments.length === 1 ){
            ...
            // 如果是对象，把对象中所有的属性节点添加到所有的元素中
                ...
                jQuery.each( attr, function( key, val ){
                    ...
                    // 我们要使用的就是基本数据类型，而this是基本数据类型的包装类型(对象)
                    // 所以这里不能使用this。
    ...
    prop: function( attr, val ){
       ...
       * 1、判断attr是不是字符串或者对象，不是直接return this。
       * 2、如果是字符串，那么继续判断arguments的length
       * 3、length为1，则获取第一个元素指定的属性值返回
       * 4、length>=2，则遍历所有元素，分别给他们设置新的属性值( setAttribute )
       * 5、如果不是字符串( 是对象 )，那么遍历这个对象，得到所有的属性值，
       * 6、return this;
       * */
    ...
    _prop: function( attr, val ){
         ...
         * 2、那么判断styleName是不是字符串，是则获取第一个元素指定的属性值
         * 3、判断styleName是不是对象，是则遍历这个对象得到所有的属性分别设置给所有的元素
    ...
    addClass: function( className ){
        ...
        * */
        this.each( function(){
            ...
            // 没有则添加，有则忽略
            // 这里遍历到的this是原生的DOM 需要包装成实例 然后复用实例方法
    ...
    _addClass: function( classNames ){
        ...
        * 2、外层遍历所有的元素
        * 3、内层遍历要添加的class
        * 4、依次判断遍历到的每一元素有没有遍历到的每一个要添加的class
        * 5、有则忽略，没有则添加( className += ' ' + classNames )
        * 6、处女座最后可以考虑trim一下
        * 7、链式编程返回this
        * */
