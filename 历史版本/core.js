(function( w ){
    ...
    jQuery.fn = jQuery.prototype = {
        ...
        // 获取指定下标的元素，获取到的是原生DOM
        get: function( i ){
            /*
            * 1、如果传入null或undefined，那么转换为数组返回
            * 2、如果传入的是正数，按照指定的下标获取元素返回
            * 3、如果传入的是负数，按照指定的下标倒着( this.length + 负数 )获取元素返回
            * */
        ...
        slice: function(){
            ...
            * */
            ...
            // 我们要把arguments中的每一项传递给数组的slice，所以需要借用apply平铺传递过去，
            // 最后把slice返回数组，通过jQuery工厂包装成实例返回。
            var nodes = [].slice.apply( this, arguments );  //arguments是内置的
            return jQuery( nodes );
        },
        ...
        eq: function( i ){
           ...
           * 2、如果传入的是正数，按照指定的下标获取元素，再包装成实例返回
           * 3、如果传入的是负数，按照指定的下标倒着( this.length + 负数 )标获取元素，再包装成实例返回
           * */
    ...
    jQuery.extend({
        ...
        map: function( obj, fn ){
            ...
            /*
           * 1、先判断obj是不是数组或者伪数组，( for in 遍历是无序的 不能使用它遍历数组 )
           * 2、如果是，则通过i的方式遍历这个对象
           * 3、如果不是，则通过for in的方式遍历这个对象
           * 4、在遍历的过程中，把每一次遍历到key和val分别传给回调。
           * 5、在给回调传参的时候，需要收集回调的返回值，最后把素有的返回值构成新数组返回
           * */
        ...
        ready: function( fn ){
            ...
            // 如果支持，绑定DOMContentLoaded事件 ( 有内置的addEventListener就一定有DOMContentLoaded事件事件 )

            // 如果不支持，使用attachEvent绑定onreadystatechange事件
            // 注意，需要在里面判断document.readyState === 'complete' 才执行
            // 防止fn多次执行
        ...
        // string ==> ( html || selector )
            ...
            // selector
                ...
                try {
                    ...
                    // 如果报错勒，那么手动补一个length属性，代表没有获取到任何元素
                    this.length = 0;
                }
